...
const ensureFrontSuccess = (data, fallbackMessage) => {
    ...
    updateCart(ctx, payload) {
      ...
      return new Promise((resolve, reject) => {
        if (ctx.rootState.user.profile.token) {
          ...
          updateCart(payload)
            .then((data) => {
              ensureFrontSuccess(data, "购物车更新失败");
              return findCart();
            })
            .then((data) => {
              ensureFrontSuccess(data, "购物车刷新失败");
              ctx.commit("setCart", data.result);
              resolve();
            })
            .catch((err) => {
      ...
      return new Promise((resolve, reject) => {
        if (ctx.rootState.user.profile.token) {
          ...
          insertCart({ skuId: payload.skuId, count: payload.count })
            .then((data) => {
              ensureFrontSuccess(data, "加入购物车失败");
              return findCart();
            })
            .then((data) => {
              ensureFrontSuccess(data, "购物车刷新失败");
