      ...
      <div class="item" v-for="item in commentList" :key="item.id">
        ...
        <div class="body">
          ...
          <div class="time">
            ...
            <span class="zan" :class="{ active: item._isPraised }" @click="togglePraise(item)"
...
import { findGoodsCommentInfo, findGoodsCommentList, praiseEvaluate } from "@/api/goods";
...
export default {
  ...
  setup() {
    ...
    findGoodsCommentInfo(goods.value.id).then((data) => {
      // 有图筛选已隐藏（用户评价不支持图片上传，仅 master 预置评价含图片）
    ...
    watch(
      ...
      () => {
        findGoodsCommentList(goods.value.id, reqParams).then((data) => {
          // Initialize _isPraised from API response (backend returns isPraise)
          if (data.result.items) {
            data.result.items.forEach(item => {
              item._isPraised = item.isPraise === true;
            });
          }
    ...
    // 点赞/取消点赞
    const togglePraise = (item) => {
      const newState = !item._isPraised;
      praiseEvaluate(item.id, newState).then((data) => {
        item._isPraised = data.result.isPraise;
        item.praiseCount = data.result.praiseCount;
        // Show toast
        if (data.result.isPraise) {
          Message({ type: 'success', text: '点赞成功' });
        } else {
          Message({ type: 'warn', text: '已取消点赞' });
        }
      }).catch(() => {
        Message({ type: 'error', text: '请先登录后再点赞' });
      });
    };
