      ...
      <div class="goods-info">
        ...
        <div class="spec">
          ...
          <GoodsSku
            :key="goods.id"
            :goods="goods"
            :skuId="currSku ? currSku.skuId : ''"
            @change="changeSku"
          />
          <p class="stock-state" :class="stockClass">{{ stockText }}</p>
          <!-- 数量选择组件 -->
          <XtxNumbox
            :key="goods.id"
            label="数量"
            v-model="num"
            :max="goods.availableStock ?? goods.inventory"
          />
          <!-- 按钮组件 -->
  ...
  setup() {
    ...
    const loadGoods = async (id) => {
      ...
      try {
        ...
        const defaultSku = goods.value.skus.find((item) => (item.availableStock ?? item.inventory ?? 0) > 0) || goods.value.skus[0];
        if (defaultSku) {
          currSku.value = {
            skuId: defaultSku.id,
            price: defaultSku.price,
            oldPrice: defaultSku.oldPrice,
            inventory: defaultSku.inventory,
            availableStock: defaultSku.availableStock ?? defaultSku.inventory,
            stockStatus: defaultSku.stockStatus,
            specsText: defaultSku.specs?.reduce((p, n) => `${p} ${n.name}：${n.valueName}`, "").replace(" ", ""),
          };
          goods.value.price = defaultSku.price;
          goods.value.oldPrice = defaultSku.oldPrice;
          goods.value.inventory = defaultSku.inventory;
          goods.value.availableStock = defaultSku.availableStock ?? defaultSku.inventory;
          goods.value.stockStatus = defaultSku.stockStatus;
        }
        loading.value = false;
        // 静默记录足迹
        addHistory(id).catch(() => {});
    ...
    const changeSku = (sku) => {
      if (sku.skuId) {
        ...
        goods.value.availableStock = sku.availableStock ?? sku.inventory;
        goods.value.stockStatus = sku.stockStatus;
      }
      currSku.value = sku;
    };
    ...
    const stockText = computed(() => {
      if (!goods.value) return "库存加载中";
      const availableStock = goods.value.availableStock ?? goods.value.inventory ?? 0;
      const stockStatus = goods.value.stockStatus;
      ...
      return `当前库存 ${availableStock} 件`;
    });
    const stockClass = computed(() => {
      if (!goods.value) return "in";
      if (goods.value.stockStatus === "OUT_OF_STOCK" || (goods.value.availableStock ?? goods.value.inventory ?? 0) <= 0) {
        return "out";
      }
      if (goods.value.stockStatus === "LOW_STOCK") {
        return "low";
      }
      return "in";
    });
    const insertCart = () => {
      if (currSku.value && currSku.value.skuId) {
        const availableStock = currSku.value.availableStock ?? currSku.value.inventory ?? 0;
        if (availableStock <= 0) {
          return Message({ type: "error", text: "当前 SKU 缺货" });
        }
        if (num.value > availableStock) {
          return Message({ type: "error", text: "购买数量超过当前库存" });
        }
        ...
        store
          ...
          .catch((err) => {
            Message({ type: "error", text: err?.message || "加入购物车失败" });
          });
      } else {
    ...
    return { goods, loading, loadError, changeSku, num, insertCart, route, retryLoad, stockText, stockClass };
  },
};
</script>
