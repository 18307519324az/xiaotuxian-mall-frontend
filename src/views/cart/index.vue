          ...
          <tbody>
            ...
            <tr v-for="goods in $store.getters['cart/validList']" :key="goods.skuId">
                ...
                <div class="goods">
                  ...
                  <div>
                    ...
                    <CartSku
                      ...
                      :skuId="goods.skuId"
                      :attrsText="formatAttrsText(goods.attrsText)"
                    />
                    <p class="attr stock-text">库存 {{ goods.stock }} 件</p>
                  </div>
                </div>
              </td>
              <td class="tc">
              ...
              <td class="tc">
                ...
                <p><a @click="$router.push('/similar/' + (goods.goodsId || goods.id))" href="javascript:;">找相似</a></p>
              </td>
            </tr>
          </tbody>
          <!-- 无效商品 -->
            ...
            <tr v-for="goods in $store.getters['cart/invalidList']" :key="goods.skuId">
              ...
              <td>
                <div class="goods">
                  ...
                  <div>
                    <p class="name ellipsis">{{ goods.name }}</p>
                    <p class="attr">{{ formatAttrsText(goods.attrsText) }}</p>
                  </div>
                </div>
              </td>
              <td class="tc">
...
export default {
  ...
  setup() {
    ...
    const updateCount = (skuId, count) => {
      store.dispatch("cart/updateCart", { skuId, count }).catch((err) => {
        Message({ type: "error", text: err?.message || "库存校验失败" });
    ...
    const checkout = () => {
      ...
      const invalidStockItem = store.getters["cart/selectedList"].find((item) => item.count > item.stock || item.stock <= 0);
      if (invalidStockItem) {
        return Message({ type: "error", text: `商品“${invalidStockItem.name}”库存不足，请调整后再结算` });
      }
      // 如果登录直接跳转
    ...
    return { checkOne, checkAll, deleteCart, batchDeleteCart, updateCount, updateCartSku, checkout, formatAttrsText };
  },
};
</script>
