<template>
  <HomePanel title="最新专题">
    <template v-slot:right><XtxMore path="/special" /></template>
    <div ref="target" class="special-list">
      <div class="special-item" v-for="item in specialList" :key="item.id" @click="showDetail(item)">
...
export default {
  ...
  setup() {
    ...
    // 首页专题 flex 布局固定展示 3 项（width: 404px × 3 ≈ 1240px 容器）
    const specialList = computed(() => (result.value || []).slice(0, 3));
    const showDetail = (item) => {
      router.push(`/special/${item.id}`);
    };
    return { target, specialList, showDetail };
  },
};
</script>
