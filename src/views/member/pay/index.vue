    ...
    <div class="container">
      ...
      <div class="pay-info" v-if="order">
        ...
        <div class="amount">
          ...
          <p v-if="couponSummary">优惠券：{{ couponSummary }}</p>
          <p v-if="giftCardSummary">礼品卡：{{ giftCardSummary }}</p>
        </div>
      </div>
      <div class="pay-type">
...
import { findOrderDetail, findOrderDetailByOrderNo, payOrder } from "@/api/order";
...
export default {
  ...
  setup() {
    ...
    const orderRequest = orderId ? findOrderDetail(orderId) : findOrderDetailByOrderNo(orderNo);
    orderRequest.then((data) => {
    ...
    const couponSummary = computed(() => {
      if (!order.value?.couponId) return "";
      const parts = [order.value.couponName || order.value.couponId];
      if (order.value.discountAmount && parseFloat(order.value.discountAmount) > 0) {
        parts.push(`-¥${order.value.discountAmount}`);
      }
      return parts.join(" ");
    });

    const giftCardSummary = computed(() => {
      if (!order.value?.giftCardCode) return "";
      if (order.value.giftCardAmount && parseFloat(order.value.giftCardAmount) > 0) {
        return `${order.value.giftCardCode} -¥${order.value.giftCardAmount}`;
      }
      return order.value.giftCardCode;
    });
    ...
    const confirmPayFn = () => {
      ...
      paying.value = true;
      payOrder({
        orderId: order.value?.id || orderId,
        orderNo: order.value?.orderNo,
        payChannel: selectedChannel.value,
      })
        .then(() => {
          Message({ type: "success", text: "支付成功" });
          router.push(`/member/order/${order.value?.id || orderId}`);
        })
        .catch((err) => {
