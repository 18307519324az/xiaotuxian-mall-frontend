    ...
    <div class="address-edit">
      <div class="xtx-form">
        ...
        <div class="xtx-form-item">
          ...
          <div class="field">
            <XtxCity :key="'city-' + (formData.id || 'new')" :fullLocation="formData.fullLocation" @change="changeCity" placeholder="请选择所在地区" />
          </div>
        </div>
        <div class="xtx-form-item">
  ...
  setup(props, { emit }) {
    ...
    const open = (address) => {
      if (address && address.id) {
        // 如果修改 先填充表单，后显示对话框（确保 XtxCity 挂载时 formData 已就绪）
      ...
      // 填充完成后再显示对话框，XtxCity 挂载时 formData.fullLocation 已存在
    ...
    const submit = () => {
      ...
      if (formData.id) {
        ...
        addAddress(formData).then((data) => {
          // 防御性检查：确保 data.result 和 data.result.id 存在
          const resultObj = data?.result;
          if (!resultObj || !resultObj.id) {
            Message({ type: "error", text: "添加地址失败：" + JSON.stringify(data) });
            return;
          }
          // 需要设置ID
          formData.id = resultObj.id;
          // 提示
