...
export default {
  ...
  setup(props, { emit }) {
    ...
    const successHandler = (formData) => {
      ...
      const address = formData.id ? props.list.find((item) => item.id === formData.id) : null;
      if (address) {
        for (const key in address) {
          address[key] = formData[key];
        }
        // 修改后同步更新当前选中的地址 + 重新 emit change 确保父组件 addressId 正确
        if (showAddress.value && showAddress.value.id === formData.id) {
        ...
        const newAddr = JSON.parse(jsonStr);
        // eslint-disable-next-line vue/no-mutating-props
        props.list.unshift(newAddr);
        // 自动选中新地址（确保包含 receiver/contact/fullLocation/address/id）
        showAddress.value = newAddr;
        emit("change", newAddr.id);
      }
    };
