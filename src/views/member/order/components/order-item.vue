  ...
  <div class="order-item">
    ...
    <div class="body">
        ...
        <ul>
          <li v-for="goods in order.skus" :key="goods.id">
            <RouterLink class="image" :to="`/product/${goods.spuId}`">
              <img v-img-error :src="goods.image" alt="" />
            </RouterLink>
            <div class="info">
              <p class="name ellipsis-2">{{ goods.name }}</p>
              <p class="attr ellipsis">{{ formatAttrsText(goods.attrsText) }}</p>
            </div>
      ...
      <div class="column state">
        ...
        <p v-if="order.orderState === 4"><RouterLink class="green" :to="`/member/review?orderId=${order.id}`">评价商品</RouterLink></p>
        <p v-if="order.orderState === 5"><RouterLink class="green" :to="`/member/review?orderId=${order.id}`">查看评价</RouterLink></p>
      </div>
      ...
      <div class="column action">
        ...
        <XtxButton @click="$emit('on-ship', order)" v-if="order.orderState === 2" type="primary" size="small">模拟发货</XtxButton>
        ...
        <p v-if="[4, 5].includes(order.orderState)"><RouterLink :to="`/member/after-sale?orderId=${order.id}`">申请售后</RouterLink></p>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  emits: ["on-cancel", "on-delete", "on-confirm", "on-logistics", "on-ship"],
  setup(props) {
    const { start, timeText } = usePayTime();
    start(props.order.countdown);

    return { orderStatus, timeText, formatAttrsText };
  },
};
</script>
